...
public class EngineSelect {
	...
	public String[][] selectColumnsWithCondition(ArrayList<ArrayList<String>> tableData,String[]condition,
		...
		for (int i = 0; i <itemsCounter.size(); i++) {newRow.add(tableData.get(0).get(itemsCounter.get(i)));}
		output.add(newRow);
        loopAndAdding(newRow, tableData, columType, condition, testing, itemsCounter, output);
		selectTypes= new String[itemsCounter.size()];
		for (int i = 0; i < itemsCounter.size(); i++) {selectTypes[i]=ArrayOfTypes[itemsCounter.get(i)];}
	...
	public void  loopAndAdding(ArrayList<String> newRow ,ArrayList<ArrayList<String>> tableData, String columType, String[] condition, int testing,
			ArrayList<Integer> itemsCounter,ArrayList<ArrayList<String>> output) {
	...
	public String[][]selectColumns(ArrayList<ArrayList<String>> tableData,String[]columntitles,String[] ArrayOfTypes, String[]headers){
		...
		for (int i = 0; i < tableData.size(); i++) {
			newRow= new ArrayList<String>();
			for (int j = 0; j <itemsCounterUpdate.size(); j++) {newRow.add(tableData.get(i).get(itemsCounterUpdate.get(j)));}
			if(!newRow.isEmpty() ){output.add(newRow);}
		}
		...
		String[][]outputTable = new String[(output.size())][output.get(0).size()];
		for (int i = 0; i < output.size(); i++) {
			for (int j = 0; j < output.get(0).size(); j++) {outputTable[i][j] = output.get(i).get(j);}
		}
		return outputTable;
	}
